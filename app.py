...
def create_app() -> Flask:
    ...
    def api_kisi_update(kisi_id: int):
        ...
        with get_connection() as conn:
            ...
            ad_soyad = (fields["ad_soyad"] if fields["ad_soyad"] is not None else existing["ad_soyad"]).strip()
            if not ad_soyad:
                return jsonify({"error": "ad_soyad zorunlu"}), 400
            ...
            toplam_odenen = float(
                fields["toplam_odenen"] if fields["toplam_odenen"] is not None else existing["toplam_odenen"] or 0
            )
            vekalet_durumu = int(
                fields["vekalet_durumu"] if fields["vekalet_durumu"] is not None else existing["vekalet_durumu"] or 0
            )
